...
public:
    ...
    //eval: radiance for a defined incident light 
    ...
    // sample: use for integral
    virtual Color3f sample(EmitterQueryRecord & lRec, const Point2f & sample) const override {
        if(!m_shape)
            throw NoriException("There is no shape attached to this Area light!");

        ShapeQueryRecord sRec(lRec.ref);
        m_shape->sampleSurface(sRec, sample); // m_sample: area light
        lRec.p = sRec.p;
        lRec.n = sRec.n;
    ...
    virtual float pdf(const EmitterQueryRecord &lRec) const override {
        ...
        if (cosTheta > 0) {
            ...
            float pdf = m_shape->pdfSurface(sRec); // pdf of a certain point
            auto sNorm = (lRec.ref - lRec.p).squaredNorm();
            return pdf * sNorm / cosTheta;
        }
