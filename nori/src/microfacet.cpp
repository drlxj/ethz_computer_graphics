...
public:
    ...
    virtual Color3f eval(const BSDFQueryRecord &bRec) const override {
        ...
        float G = smithBeckmannG1(wi, wh) * smithBeckmannG1(wo, wh);
        float Jh = 1 / (4 * wh.dot(wo));
        
        return m_kd * INV_PI + m_ks * D * F * G * Jh;
    ...
    virtual Color3f sample(BSDFQueryRecord &bRec, const Point2f &_sample) const override {
        ...
        if (_sample.x() < m_ks) {
            ...
            // sampling a normal from the microfacet distribution (= beckmann)
            wh = Warp::squareToBeckmann(sample, m_alpha);
            // reflect incident direction wi using wh
            wo = (2 * wi.dot(wh) * wh - wi).normalized();
        }
        else {
            Point2f sample((_sample.x() - m_ks) / (1 - m_ks), _sample.y());
            // generate a cosine-weighted direction on the sphere using the same method as the model in src/diffuse.cpp
